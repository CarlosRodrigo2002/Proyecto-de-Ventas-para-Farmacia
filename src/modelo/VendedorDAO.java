...
public class VendedorDAO implements CRUD {
    ...
    public Vendedor iniciarSesion(String user, String dui) {
        Vendedor entidadVendedor = new Vendedor();
        String sql = "select * from vendedor where user_vendedor = ? and dui_vendedor = ?;";
        try {
            ...
            while (rs.next()) {
                ...
                entidadVendedor.setDui_vendedor(rs.getString(2));
    ...
    public int agregar(Object[] object) {
        int respuesta = 0;
        String sql = "insert into vendedor(dui_vendedor, nombre_vendedor, celular_vendedor, estado_vendedor, user_vendedor) values (?, ?, ?, ?, ?);";
        try {
            connection = conexion.conectar();
            ps = connection.prepareStatement(sql);
    ...
    public List listar() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                vendedor.setDui_vendedor(rs.getString(2));
    ...
    public int actualizar(Object[] object) {
        int respuesta = 0;
        String sql = "update vendedor set dui_vendedor=?, nombre_vendedor=?, celular_vendedor=?, estado_vendedor=?, user_vendedor=? where id_vendedor=?;";
        try {
            connection = conexion.conectar();
            ps = connection.prepareStatement(sql);
            ps.setObject(1, object[0]);
